...
public class Token : MonoBehaviour, IBeginDragHandler, IDragHandler, IEndDragHandler
{
	...
	public void OnDrag(PointerEventData eventData)
	{
		...
		float currentFrameTokenDrag = Vector3.Distance(previousPosition,
        transform.position);
		...
		float interpolatedDrag = Mathf.Lerp(m_audioSource.volume, clampedVolumeDrag -
		 0.2f, 0.7f);

		// умножаем на общий уровень звука, чтобы регулятор громкости
		// (когда мы его добавим) влиял на этот звук
		m_audioSource.volume = interpolatedDrag *
	...
	private void GetDragSpace()
	{
		...
		m_dragSpace[2] = CheckSpace(Vector2.up); if (m_dragSpace[2] == -1)
		{
