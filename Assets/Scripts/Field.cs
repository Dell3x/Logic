...
public class Field : MonoBehaviour
{
    ...
    public void CreateTokens(int size, int emptySquares)
    {
        //Положение первой фишки - левый нижний угол:

        float offset = ((float)size - 1f) / 2f;

        Vector3 startPosition = new Vector3(transform.position.x - offset,
        ...
        for (int i = 0; i < size; i++)
        {
            for (int j = 0; j < size; j++)
            {
                ...
                else
                {
                    ...
                    if (emptySquares == 0 ||
                       Random.Range(0, size * size / emptySquares) > 0)
                    {
                        //создаем новую фишку
                        Token newToken =
                        Instantiate(Resources.Load("Prefabs/Token"),
                        new Vector3(startPosition.x + i, startPosition.y + j, startPosition.z), Quaternion.identity) as Token;
                    }
                    //Иначе
                    else
                    {
